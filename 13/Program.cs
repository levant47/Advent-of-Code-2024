public static class Program
{
    public static void Main()
    {
        Console.WriteLine("Starting problem 13...");

        var input = INPUT;
        var games = ParseGames(input);
        Console.WriteLine($"Part 1 answer: {games.Select(SolveGame2).Where(cost => cost != long.MaxValue).Sum()}");
        foreach (var game in games) { game.Prize = new(game.Prize.X + 10000000000000, game.Prize.Y + 10000000000000); }
        Console.WriteLine($"Part 2 answer: {games.Select(SolveGame2).Where(cost => cost != long.MaxValue).Sum()}");

        Console.WriteLine("Done");
    }

    public record Vector2(long X, long Y)
    {
        public static Vector2 operator*(Vector2 vector, long factor) => new(vector.X * factor, vector.Y * factor);
        public static Vector2 operator*(long factor, Vector2 vector) => vector * factor;
        public static Vector2 operator-(Vector2 left, Vector2 right) => new(left.X - right.X, left.Y - right.Y);
        public static Vector2 operator+(Vector2 left, Vector2 right) => new(left.X + right.X, left.Y + right.Y);
    }

    public class Game
    {
        public Vector2 AStep;
        public Vector2 BStep;
        public Vector2 Prize;
    }

    public static long SolveGame2(Game game) => SolveEquation(game.AStep.X, game.BStep.X, game.Prize.X)
        .Intersect(SolveEquation(game.AStep.Y, game.BStep.Y, game.Prize.Y))
        .Select(solution => solution.X * 3 + solution.Y)
        .DefaultIfEmpty(long.MaxValue)
        .Min();

    public static List<Vector2> SolveEquation(long a, long b, long c)
    {
        while (true)
        {
            var (gcd, _, _) = Egcd(a, Egcd(b, c).Item1);
            if (gcd == 1) { break; }
            a /= gcd;
            b /= gcd;
            c /= gcd;
        }
        var inv = ModularInverse(a, b);
        if (inv == null) { return []; }
        var d = Mod(c * (long)inv, b);
        var edge1 = (long)Math.Ceiling((double)-d / b);
        var edge2 = (long)Math.Ceiling(((double)c - a * d) / (a * b));
        var result = new List<Vector2>();
        for (var k = Math.Min(edge1, edge2); k < Math.Max(edge1, edge2); k++)
        {
            var n = d + b * k;
            var m = (c - a * n) / b;
            result.Add(new(n, m));
        }
        return result;
    }

    public static long? ModularInverse(long dividend, long divisor)
    {
        var (gcd, x, _) = Egcd(dividend, divisor);
        if (gcd != 1) { return null; }
        return Mod(x, divisor);
    }

    public static (long, long, long) Egcd(long a, long b)
    {
        if (b == 0) { return (a, 1, 0); }
        var (gcd, x1, y1) = Egcd(b, a % b);
        var x = y1;
        var y = x1 - (a / b) * y1;
        return (gcd, x, y);
    }

    public static long Mod(long dividend, long divisor)
    {
        if (dividend == 0 || divisor == 0) { return 0; }
        var result = dividend % divisor;
        if (Math.Sign(dividend) != Math.Sign(divisor)) { result += divisor; }
        return result;
    }

    public static List<Game> ParseGames(string source)
    {
        var lines = source.Trim().Split('\n').Select(line => line.Trim()).ToList();
        var result = new List<Game>();
        for (var i = 0; i < lines.Count; i++)
        {
            var ax = long.Parse(lines[i].Between("X+", ","));
            var ay = long.Parse(lines[i].After("Y+"));
            var bx = long.Parse(lines[i + 1].Between("X+", ","));
            var by = long.Parse(lines[i + 1].After("Y+"));
            var prizeX = long.Parse(lines[i + 2].Between("X=", ","));
            var prizeY = long.Parse(lines[i + 2].After("Y="));
            result.Add(new() { AStep = new(ax, ay), BStep = new(bx, by), Prize = new(prizeX, prizeY) });
            i += 3;
        }
        return result;
    }

    public static string Between(this string source, string prefix, string suffix)
    {
        var prefixIndex = source.IndexOf(prefix);
        if (prefixIndex == -1) { throw new(); }
        var suffixIndex = source.IndexOf(suffix, prefixIndex);
        if (suffixIndex == -1) { throw new(); }
        return source[(prefixIndex + prefix.Length)..suffixIndex];
    }

    public static string After(this string source, string prefix)
    {
        var prefixIndex = source.IndexOf(prefix);
        if (prefixIndex == -1) { throw new(); }
        return source[(prefixIndex + prefix.Length)..];
    }

    public const string EXAMPLE_INPUT = """
    Button A: X+94, Y+34
    Button B: X+22, Y+67
    Prize: X=8400, Y=5400

    Button A: X+26, Y+66
    Button B: X+67, Y+21
    Prize: X=12748, Y=12176

    Button A: X+17, Y+86
    Button B: X+84, Y+37
    Prize: X=7870, Y=6450

    Button A: X+69, Y+23
    Button B: X+27, Y+71
    Prize: X=18641, Y=10279
    """;

    public const string INPUT = """
    Button A: X+53, Y+35
    Button B: X+11, Y+23
    Prize: X=13386, Y=18840

    Button A: X+57, Y+35
    Button B: X+19, Y+39
    Prize: X=12698, Y=18002

    Button A: X+62, Y+29
    Button B: X+45, Y+65
    Prize: X=4448, Y=4366

    Button A: X+90, Y+19
    Button B: X+48, Y+75
    Prize: X=6270, Y=3594

    Button A: X+98, Y+19
    Button B: X+35, Y+45
    Prize: X=7133, Y=3714

    Button A: X+26, Y+22
    Button B: X+13, Y+96
    Prize: X=2288, Y=9416

    Button A: X+24, Y+19
    Button B: X+29, Y+80
    Prize: X=4054, Y=6746

    Button A: X+85, Y+55
    Button B: X+39, Y+85
    Prize: X=7812, Y=8820

    Button A: X+50, Y+21
    Button B: X+14, Y+28
    Prize: X=16748, Y=18645

    Button A: X+19, Y+48
    Button B: X+56, Y+35
    Prize: X=7401, Y=19267

    Button A: X+12, Y+87
    Button B: X+81, Y+66
    Prize: X=4092, Y=6732

    Button A: X+37, Y+14
    Button B: X+14, Y+52
    Prize: X=9288, Y=10480

    Button A: X+86, Y+45
    Button B: X+48, Y+86
    Prize: X=4280, Y=4614

    Button A: X+38, Y+89
    Button B: X+93, Y+52
    Prize: X=10769, Y=9138

    Button A: X+86, Y+11
    Button B: X+64, Y+73
    Prize: X=7738, Y=4036

    Button A: X+46, Y+32
    Button B: X+23, Y+84
    Prize: X=3473, Y=4524

    Button A: X+13, Y+95
    Button B: X+79, Y+74
    Prize: X=4673, Y=4957

    Button A: X+11, Y+29
    Button B: X+51, Y+36
    Prize: X=5976, Y=7050

    Button A: X+24, Y+50
    Button B: X+57, Y+16
    Prize: X=7772, Y=10678

    Button A: X+20, Y+31
    Button B: X+46, Y+20
    Prize: X=5710, Y=7236

    Button A: X+21, Y+83
    Button B: X+73, Y+28
    Prize: X=1962, Y=6973

    Button A: X+59, Y+39
    Button B: X+22, Y+72
    Prize: X=2874, Y=6324

    Button A: X+88, Y+47
    Button B: X+12, Y+24
    Prize: X=1848, Y=1761

    Button A: X+14, Y+34
    Button B: X+66, Y+24
    Prize: X=3004, Y=10704

    Button A: X+59, Y+14
    Button B: X+35, Y+81
    Prize: X=3587, Y=7321

    Button A: X+21, Y+70
    Button B: X+97, Y+85
    Prize: X=6916, Y=6370

    Button A: X+20, Y+36
    Button B: X+42, Y+16
    Prize: X=7756, Y=6896

    Button A: X+95, Y+26
    Button B: X+25, Y+93
    Prize: X=9255, Y=3653

    Button A: X+86, Y+48
    Button B: X+12, Y+64
    Prize: X=5052, Y=4768

    Button A: X+32, Y+72
    Button B: X+98, Y+64
    Prize: X=8008, Y=7376

    Button A: X+58, Y+93
    Button B: X+40, Y+12
    Prize: X=2904, Y=2988

    Button A: X+26, Y+58
    Button B: X+50, Y+18
    Prize: X=14824, Y=744

    Button A: X+21, Y+88
    Button B: X+89, Y+89
    Prize: X=6676, Y=7815

    Button A: X+62, Y+43
    Button B: X+12, Y+28
    Prize: X=3770, Y=11095

    Button A: X+92, Y+52
    Button B: X+13, Y+73
    Prize: X=4940, Y=3580

    Button A: X+24, Y+49
    Button B: X+53, Y+25
    Prize: X=3554, Y=9129

    Button A: X+12, Y+33
    Button B: X+43, Y+30
    Prize: X=19546, Y=3989

    Button A: X+30, Y+64
    Button B: X+43, Y+26
    Prize: X=3611, Y=4614

    Button A: X+36, Y+42
    Button B: X+16, Y+97
    Prize: X=4444, Y=12313

    Button A: X+66, Y+21
    Button B: X+13, Y+44
    Prize: X=3591, Y=775

    Button A: X+71, Y+17
    Button B: X+79, Y+76
    Prize: X=5591, Y=4250

    Button A: X+15, Y+37
    Button B: X+72, Y+25
    Prize: X=1872, Y=2939

    Button A: X+21, Y+50
    Button B: X+50, Y+31
    Prize: X=8024, Y=6548

    Button A: X+72, Y+99
    Button B: X+83, Y+13
    Prize: X=11442, Y=7845

    Button A: X+32, Y+12
    Button B: X+63, Y+85
    Prize: X=14461, Y=3519

    Button A: X+33, Y+59
    Button B: X+23, Y+13
    Prize: X=13887, Y=5765

    Button A: X+50, Y+21
    Button B: X+27, Y+51
    Prize: X=2429, Y=17498

    Button A: X+22, Y+75
    Button B: X+61, Y+12
    Prize: X=13447, Y=13595

    Button A: X+67, Y+37
    Button B: X+15, Y+29
    Prize: X=16951, Y=1877

    Button A: X+82, Y+95
    Button B: X+97, Y+20
    Prize: X=5415, Y=3225

    Button A: X+13, Y+30
    Button B: X+93, Y+50
    Prize: X=9652, Y=6800

    Button A: X+20, Y+28
    Button B: X+39, Y+16
    Prize: X=16877, Y=17536

    Button A: X+46, Y+16
    Button B: X+13, Y+23
    Prize: X=13392, Y=2282

    Button A: X+69, Y+37
    Button B: X+20, Y+48
    Prize: X=9614, Y=11250

    Button A: X+13, Y+49
    Button B: X+59, Y+14
    Prize: X=19015, Y=3427

    Button A: X+13, Y+37
    Button B: X+41, Y+14
    Prize: X=12739, Y=2356

    Button A: X+54, Y+16
    Button B: X+20, Y+59
    Prize: X=7018, Y=9826

    Button A: X+59, Y+11
    Button B: X+19, Y+78
    Prize: X=9180, Y=11088

    Button A: X+45, Y+18
    Button B: X+43, Y+61
    Prize: X=4435, Y=2212

    Button A: X+87, Y+50
    Button B: X+32, Y+84
    Prize: X=5376, Y=8404

    Button A: X+43, Y+71
    Button B: X+25, Y+11
    Prize: X=18994, Y=2502

    Button A: X+28, Y+55
    Button B: X+41, Y+18
    Prize: X=4778, Y=6424

    Button A: X+64, Y+25
    Button B: X+28, Y+48
    Prize: X=1936, Y=2980

    Button A: X+49, Y+19
    Button B: X+23, Y+45
    Prize: X=1313, Y=18963

    Button A: X+28, Y+62
    Button B: X+55, Y+19
    Prize: X=9538, Y=11506

    Button A: X+30, Y+12
    Button B: X+13, Y+47
    Prize: X=3851, Y=4907

    Button A: X+41, Y+72
    Button B: X+70, Y+38
    Prize: X=5243, Y=5980

    Button A: X+50, Y+17
    Button B: X+31, Y+63
    Prize: X=8695, Y=10046

    Button A: X+16, Y+39
    Button B: X+19, Y+11
    Prize: X=8118, Y=11867

    Button A: X+12, Y+68
    Button B: X+72, Y+19
    Prize: X=6980, Y=12648

    Button A: X+30, Y+86
    Button B: X+73, Y+34
    Prize: X=7539, Y=5312

    Button A: X+12, Y+38
    Button B: X+97, Y+23
    Prize: X=1830, Y=680

    Button A: X+57, Y+31
    Button B: X+28, Y+85
    Prize: X=3762, Y=6023

    Button A: X+90, Y+62
    Button B: X+23, Y+79
    Prize: X=3643, Y=5099

    Button A: X+31, Y+72
    Button B: X+44, Y+15
    Prize: X=6324, Y=16028

    Button A: X+11, Y+60
    Button B: X+89, Y+29
    Prize: X=889, Y=741

    Button A: X+48, Y+28
    Button B: X+13, Y+91
    Prize: X=1664, Y=3640

    Button A: X+64, Y+21
    Button B: X+14, Y+58
    Prize: X=4076, Y=15941

    Button A: X+38, Y+17
    Button B: X+11, Y+49
    Prize: X=14289, Y=7126

    Button A: X+66, Y+33
    Button B: X+19, Y+52
    Prize: X=2616, Y=7533

    Button A: X+63, Y+82
    Button B: X+91, Y+25
    Prize: X=10563, Y=7301

    Button A: X+60, Y+30
    Button B: X+34, Y+66
    Prize: X=11898, Y=8912

    Button A: X+65, Y+33
    Button B: X+11, Y+78
    Prize: X=7122, Y=10278

    Button A: X+68, Y+32
    Button B: X+11, Y+29
    Prize: X=4552, Y=4048

    Button A: X+50, Y+36
    Button B: X+28, Y+75
    Prize: X=2896, Y=2469

    Button A: X+36, Y+66
    Button B: X+36, Y+12
    Prize: X=3824, Y=16262

    Button A: X+35, Y+24
    Button B: X+12, Y+65
    Prize: X=1203, Y=1052

    Button A: X+77, Y+26
    Button B: X+66, Y+79
    Prize: X=8998, Y=7065

    Button A: X+80, Y+21
    Button B: X+53, Y+61
    Prize: X=9035, Y=6845

    Button A: X+36, Y+17
    Button B: X+23, Y+51
    Prize: X=16060, Y=14915

    Button A: X+80, Y+18
    Button B: X+11, Y+50
    Prize: X=4760, Y=11860

    Button A: X+39, Y+94
    Button B: X+65, Y+26
    Prize: X=4940, Y=6288

    Button A: X+58, Y+16
    Button B: X+39, Y+80
    Prize: X=16872, Y=7232

    Button A: X+46, Y+19
    Button B: X+50, Y+80
    Prize: X=3782, Y=2393

    Button A: X+38, Y+76
    Button B: X+51, Y+19
    Prize: X=1857, Y=1805

    Button A: X+41, Y+20
    Button B: X+29, Y+94
    Prize: X=5904, Y=9428

    Button A: X+20, Y+33
    Button B: X+41, Y+18
    Prize: X=19032, Y=2225

    Button A: X+61, Y+27
    Button B: X+19, Y+48
    Prize: X=9809, Y=2663

    Button A: X+43, Y+70
    Button B: X+35, Y+16
    Prize: X=2813, Y=5334

    Button A: X+84, Y+19
    Button B: X+11, Y+76
    Prize: X=880, Y=13230

    Button A: X+51, Y+20
    Button B: X+14, Y+39
    Prize: X=9175, Y=14274

    Button A: X+33, Y+20
    Button B: X+20, Y+39
    Prize: X=9845, Y=11893

    Button A: X+42, Y+60
    Button B: X+75, Y+23
    Prize: X=4449, Y=2401

    Button A: X+12, Y+63
    Button B: X+82, Y+32
    Prize: X=3222, Y=7249

    Button A: X+71, Y+39
    Button B: X+21, Y+56
    Prize: X=7633, Y=18263

    Button A: X+12, Y+54
    Button B: X+83, Y+29
    Prize: X=10556, Y=14036

    Button A: X+32, Y+19
    Button B: X+22, Y+45
    Prize: X=3046, Y=2096

    Button A: X+41, Y+14
    Button B: X+21, Y+31
    Prize: X=9594, Y=18484

    Button A: X+45, Y+69
    Button B: X+88, Y+42
    Prize: X=8387, Y=7377

    Button A: X+48, Y+19
    Button B: X+34, Y+59
    Prize: X=6862, Y=13588

    Button A: X+25, Y+80
    Button B: X+76, Y+65
    Prize: X=3896, Y=4270

    Button A: X+26, Y+94
    Button B: X+86, Y+20
    Prize: X=7332, Y=3816

    Button A: X+21, Y+49
    Button B: X+98, Y+71
    Prize: X=3332, Y=2887

    Button A: X+28, Y+56
    Button B: X+52, Y+28
    Prize: X=3780, Y=4932

    Button A: X+57, Y+43
    Button B: X+15, Y+54
    Prize: X=2520, Y=2584

    Button A: X+86, Y+36
    Button B: X+61, Y+93
    Prize: X=6006, Y=2784

    Button A: X+70, Y+13
    Button B: X+18, Y+77
    Prize: X=6296, Y=13842

    Button A: X+84, Y+27
    Button B: X+56, Y+69
    Prize: X=5264, Y=2661

    Button A: X+11, Y+25
    Button B: X+50, Y+11
    Prize: X=19810, Y=5751

    Button A: X+19, Y+50
    Button B: X+21, Y+11
    Prize: X=14441, Y=18206

    Button A: X+27, Y+62
    Button B: X+65, Y+22
    Prize: X=12928, Y=8592

    Button A: X+68, Y+22
    Button B: X+67, Y+82
    Prize: X=10281, Y=6644

    Button A: X+40, Y+65
    Button B: X+34, Y+18
    Prize: X=4708, Y=6086

    Button A: X+16, Y+53
    Button B: X+58, Y+13
    Prize: X=8520, Y=11920

    Button A: X+99, Y+27
    Button B: X+72, Y+79
    Prize: X=13707, Y=8428

    Button A: X+41, Y+14
    Button B: X+21, Y+44
    Prize: X=14527, Y=13878

    Button A: X+31, Y+11
    Button B: X+27, Y+45
    Prize: X=13928, Y=6032

    Button A: X+25, Y+54
    Button B: X+60, Y+23
    Prize: X=900, Y=7491

    Button A: X+43, Y+78
    Button B: X+52, Y+20
    Prize: X=17700, Y=3516

    Button A: X+15, Y+32
    Button B: X+44, Y+15
    Prize: X=9890, Y=7278

    Button A: X+16, Y+35
    Button B: X+74, Y+50
    Prize: X=15554, Y=13400

    Button A: X+38, Y+71
    Button B: X+94, Y+16
    Prize: X=4276, Y=5116

    Button A: X+24, Y+48
    Button B: X+57, Y+33
    Prize: X=16631, Y=5759

    Button A: X+14, Y+34
    Button B: X+62, Y+19
    Prize: X=17908, Y=5151

    Button A: X+51, Y+13
    Button B: X+26, Y+56
    Prize: X=9069, Y=8243

    Button A: X+49, Y+81
    Button B: X+97, Y+15
    Prize: X=10767, Y=8787

    Button A: X+21, Y+75
    Button B: X+74, Y+21
    Prize: X=10175, Y=11444

    Button A: X+55, Y+94
    Button B: X+69, Y+19
    Prize: X=5011, Y=6190

    Button A: X+34, Y+15
    Button B: X+11, Y+29
    Prize: X=17791, Y=19222

    Button A: X+58, Y+23
    Button B: X+35, Y+69
    Prize: X=7256, Y=2377

    Button A: X+37, Y+84
    Button B: X+88, Y+51
    Prize: X=5035, Y=7860

    Button A: X+12, Y+75
    Button B: X+82, Y+11
    Prize: X=9268, Y=17544

    Button A: X+14, Y+53
    Button B: X+40, Y+21
    Prize: X=2490, Y=9657

    Button A: X+88, Y+36
    Button B: X+12, Y+42
    Prize: X=3992, Y=2820

    Button A: X+27, Y+11
    Button B: X+12, Y+24
    Prize: X=275, Y=2867

    Button A: X+30, Y+53
    Button B: X+95, Y+54
    Prize: X=6580, Y=6616

    Button A: X+17, Y+42
    Button B: X+67, Y+27
    Prize: X=13750, Y=16175

    Button A: X+24, Y+75
    Button B: X+39, Y+12
    Prize: X=1793, Y=13514

    Button A: X+50, Y+26
    Button B: X+18, Y+34
    Prize: X=16886, Y=15478

    Button A: X+24, Y+84
    Button B: X+47, Y+13
    Prize: X=3348, Y=6264

    Button A: X+18, Y+97
    Button B: X+83, Y+17
    Prize: X=7870, Y=2825

    Button A: X+65, Y+22
    Button B: X+11, Y+31
    Prize: X=4764, Y=2949

    Button A: X+80, Y+14
    Button B: X+77, Y+93
    Prize: X=2266, Y=1828

    Button A: X+14, Y+52
    Button B: X+68, Y+11
    Prize: X=19444, Y=7189

    Button A: X+31, Y+14
    Button B: X+21, Y+50
    Prize: X=1023, Y=10942

    Button A: X+26, Y+68
    Button B: X+47, Y+15
    Prize: X=19560, Y=9422

    Button A: X+50, Y+19
    Button B: X+24, Y+59
    Prize: X=4686, Y=9547

    Button A: X+47, Y+30
    Button B: X+18, Y+40
    Prize: X=18351, Y=9410

    Button A: X+13, Y+82
    Button B: X+84, Y+12
    Prize: X=2131, Y=17074

    Button A: X+40, Y+63
    Button B: X+44, Y+18
    Prize: X=6724, Y=16721

    Button A: X+14, Y+34
    Button B: X+72, Y+34
    Prize: X=5358, Y=19794

    Button A: X+13, Y+93
    Button B: X+71, Y+79
    Prize: X=2984, Y=5048

    Button A: X+54, Y+21
    Button B: X+45, Y+87
    Prize: X=3771, Y=4872

    Button A: X+52, Y+16
    Button B: X+51, Y+64
    Prize: X=8179, Y=6816

    Button A: X+43, Y+21
    Button B: X+35, Y+52
    Prize: X=1065, Y=8212

    Button A: X+98, Y+30
    Button B: X+29, Y+95
    Prize: X=6018, Y=10110

    Button A: X+77, Y+29
    Button B: X+19, Y+61
    Prize: X=18548, Y=764

    Button A: X+17, Y+42
    Button B: X+56, Y+28
    Prize: X=12014, Y=872

    Button A: X+62, Y+25
    Button B: X+12, Y+48
    Prize: X=860, Y=1210

    Button A: X+42, Y+12
    Button B: X+26, Y+60
    Prize: X=4792, Y=3872

    Button A: X+19, Y+43
    Button B: X+66, Y+40
    Prize: X=17818, Y=7888

    Button A: X+15, Y+83
    Button B: X+72, Y+13
    Prize: X=11900, Y=6871

    Button A: X+84, Y+15
    Button B: X+32, Y+94
    Prize: X=10428, Y=7689

    Button A: X+44, Y+69
    Button B: X+41, Y+13
    Prize: X=15438, Y=17599

    Button A: X+61, Y+12
    Button B: X+14, Y+40
    Prize: X=9312, Y=6208

    Button A: X+57, Y+13
    Button B: X+89, Y+96
    Prize: X=926, Y=514

    Button A: X+13, Y+31
    Button B: X+31, Y+19
    Prize: X=13328, Y=10754

    Button A: X+63, Y+27
    Button B: X+29, Y+92
    Prize: X=6381, Y=9180

    Button A: X+80, Y+32
    Button B: X+27, Y+51
    Prize: X=1603, Y=1003

    Button A: X+15, Y+68
    Button B: X+74, Y+26
    Prize: X=15281, Y=15880

    Button A: X+23, Y+53
    Button B: X+40, Y+13
    Prize: X=17000, Y=8297

    Button A: X+56, Y+79
    Button B: X+75, Y+25
    Prize: X=7992, Y=6103

    Button A: X+25, Y+38
    Button B: X+61, Y+12
    Prize: X=5445, Y=2626

    Button A: X+58, Y+12
    Button B: X+20, Y+57
    Prize: X=1558, Y=2754

    Button A: X+55, Y+36
    Button B: X+18, Y+77
    Prize: X=940, Y=3224

    Button A: X+38, Y+77
    Button B: X+89, Y+59
    Prize: X=2675, Y=4571

    Button A: X+26, Y+64
    Button B: X+72, Y+48
    Prize: X=5834, Y=8416

    Button A: X+76, Y+24
    Button B: X+22, Y+46
    Prize: X=6986, Y=4354

    Button A: X+11, Y+24
    Button B: X+91, Y+60
    Prize: X=8475, Y=6576

    Button A: X+30, Y+56
    Button B: X+53, Y+15
    Prize: X=3225, Y=3057

    Button A: X+43, Y+73
    Button B: X+45, Y+13
    Prize: X=2180, Y=17082

    Button A: X+12, Y+60
    Button B: X+87, Y+16
    Prize: X=7638, Y=5508

    Button A: X+81, Y+99
    Button B: X+77, Y+20
    Prize: X=8966, Y=5993

    Button A: X+71, Y+19
    Button B: X+32, Y+79
    Prize: X=1297, Y=2108

    Button A: X+23, Y+54
    Button B: X+45, Y+16
    Prize: X=14323, Y=15488

    Button A: X+29, Y+89
    Button B: X+91, Y+73
    Prize: X=2781, Y=4203

    Button A: X+25, Y+85
    Button B: X+56, Y+46
    Prize: X=3221, Y=8641

    Button A: X+54, Y+14
    Button B: X+16, Y+63
    Prize: X=14198, Y=1138

    Button A: X+19, Y+48
    Button B: X+59, Y+27
    Prize: X=646, Y=5318

    Button A: X+52, Y+32
    Button B: X+15, Y+33
    Prize: X=12307, Y=309

    Button A: X+92, Y+27
    Button B: X+36, Y+75
    Prize: X=5940, Y=6447

    Button A: X+11, Y+54
    Button B: X+58, Y+41
    Prize: X=2873, Y=4111

    Button A: X+11, Y+91
    Button B: X+74, Y+16
    Prize: X=4164, Y=9408

    Button A: X+12, Y+56
    Button B: X+69, Y+15
    Prize: X=15590, Y=16990

    Button A: X+60, Y+25
    Button B: X+17, Y+40
    Prize: X=4359, Y=9505

    Button A: X+45, Y+15
    Button B: X+21, Y+74
    Prize: X=3384, Y=5751

    Button A: X+71, Y+11
    Button B: X+67, Y+54
    Prize: X=11931, Y=5687

    Button A: X+23, Y+68
    Button B: X+72, Y+18
    Prize: X=4599, Y=288

    Button A: X+24, Y+55
    Button B: X+45, Y+15
    Prize: X=13142, Y=17155

    Button A: X+26, Y+58
    Button B: X+33, Y+12
    Prize: X=4482, Y=4198

    Button A: X+11, Y+53
    Button B: X+57, Y+17
    Prize: X=9011, Y=14505

    Button A: X+85, Y+49
    Button B: X+15, Y+28
    Prize: X=8360, Y=5187

    Button A: X+35, Y+13
    Button B: X+13, Y+36
    Prize: X=12734, Y=15532

    Button A: X+13, Y+39
    Button B: X+78, Y+44
    Prize: X=8164, Y=6632

    Button A: X+78, Y+51
    Button B: X+27, Y+75
    Prize: X=2172, Y=3714

    Button A: X+54, Y+14
    Button B: X+32, Y+81
    Prize: X=538, Y=13823

    Button A: X+18, Y+52
    Button B: X+95, Y+70
    Prize: X=7772, Y=9368

    Button A: X+13, Y+82
    Button B: X+76, Y+78
    Prize: X=2603, Y=9194

    Button A: X+30, Y+60
    Button B: X+32, Y+18
    Prize: X=3524, Y=12146

    Button A: X+11, Y+83
    Button B: X+87, Y+78
    Prize: X=4478, Y=8915

    Button A: X+64, Y+14
    Button B: X+16, Y+63
    Prize: X=2416, Y=3325

    Button A: X+47, Y+12
    Button B: X+50, Y+87
    Prize: X=5655, Y=8975

    Button A: X+86, Y+44
    Button B: X+15, Y+52
    Prize: X=1721, Y=1900

    Button A: X+66, Y+11
    Button B: X+81, Y+67
    Prize: X=3501, Y=1065

    Button A: X+71, Y+13
    Button B: X+41, Y+83
    Prize: X=7586, Y=6598

    Button A: X+15, Y+32
    Button B: X+27, Y+14
    Prize: X=17672, Y=6940

    Button A: X+47, Y+74
    Button B: X+77, Y+12
    Prize: X=5265, Y=7088

    Button A: X+40, Y+67
    Button B: X+40, Y+15
    Prize: X=10200, Y=16809

    Button A: X+94, Y+81
    Button B: X+11, Y+79
    Prize: X=6908, Y=10402

    Button A: X+16, Y+96
    Button B: X+74, Y+69
    Prize: X=3486, Y=7791

    Button A: X+64, Y+17
    Button B: X+33, Y+63
    Prize: X=2316, Y=1266

    Button A: X+36, Y+21
    Button B: X+17, Y+48
    Prize: X=2272, Y=1478

    Button A: X+84, Y+23
    Button B: X+12, Y+62
    Prize: X=17156, Y=3886

    Button A: X+15, Y+65
    Button B: X+62, Y+11
    Prize: X=2006, Y=4108

    Button A: X+77, Y+42
    Button B: X+12, Y+51
    Prize: X=4828, Y=13100

    Button A: X+11, Y+49
    Button B: X+52, Y+18
    Prize: X=5750, Y=14200

    Button A: X+23, Y+58
    Button B: X+71, Y+34
    Prize: X=10022, Y=15596

    Button A: X+24, Y+44
    Button B: X+40, Y+18
    Prize: X=3568, Y=10058

    Button A: X+59, Y+11
    Button B: X+19, Y+75
    Prize: X=8550, Y=7606

    Button A: X+52, Y+25
    Button B: X+41, Y+66
    Prize: X=18087, Y=13277

    Button A: X+38, Y+14
    Button B: X+29, Y+47
    Prize: X=12227, Y=7001

    Button A: X+95, Y+24
    Button B: X+53, Y+55
    Prize: X=8917, Y=5748

    Button A: X+16, Y+48
    Button B: X+53, Y+20
    Prize: X=11109, Y=5156

    Button A: X+25, Y+46
    Button B: X+36, Y+17
    Prize: X=9825, Y=17335

    Button A: X+24, Y+56
    Button B: X+74, Y+42
    Prize: X=4386, Y=4162

    Button A: X+25, Y+78
    Button B: X+45, Y+18
    Prize: X=5010, Y=4860

    Button A: X+72, Y+86
    Button B: X+83, Y+17
    Prize: X=4023, Y=1109

    Button A: X+38, Y+16
    Button B: X+32, Y+58
    Prize: X=9958, Y=4062

    Button A: X+12, Y+66
    Button B: X+76, Y+79
    Prize: X=7292, Y=7901

    Button A: X+25, Y+62
    Button B: X+45, Y+17
    Prize: X=7715, Y=6630

    Button A: X+60, Y+25
    Button B: X+23, Y+67
    Prize: X=7223, Y=13927

    Button A: X+79, Y+55
    Button B: X+13, Y+78
    Prize: X=4632, Y=4328

    Button A: X+20, Y+84
    Button B: X+87, Y+65
    Prize: X=662, Y=978

    Button A: X+22, Y+62
    Button B: X+69, Y+25
    Prize: X=11534, Y=17094

    Button A: X+64, Y+71
    Button B: X+42, Y+13
    Prize: X=6310, Y=5690

    Button A: X+84, Y+85
    Button B: X+99, Y+23
    Prize: X=10965, Y=6542

    Button A: X+32, Y+18
    Button B: X+14, Y+24
    Prize: X=16122, Y=968

    Button A: X+54, Y+54
    Button B: X+15, Y+56
    Prize: X=6399, Y=10458

    Button A: X+14, Y+43
    Button B: X+43, Y+28
    Prize: X=7512, Y=5016

    Button A: X+19, Y+51
    Button B: X+45, Y+17
    Prize: X=15569, Y=18213

    Button A: X+28, Y+15
    Button B: X+26, Y+53
    Prize: X=19000, Y=3362

    Button A: X+72, Y+30
    Button B: X+17, Y+46
    Prize: X=6139, Y=6300

    Button A: X+28, Y+39
    Button B: X+29, Y+13
    Prize: X=4125, Y=3965

    Button A: X+71, Y+11
    Button B: X+19, Y+84
    Prize: X=15516, Y=11946

    Button A: X+14, Y+58
    Button B: X+65, Y+17
    Prize: X=2804, Y=9164

    Button A: X+83, Y+53
    Button B: X+28, Y+88
    Prize: X=6175, Y=5065

    Button A: X+17, Y+83
    Button B: X+89, Y+49
    Prize: X=2043, Y=1493

    Button A: X+47, Y+18
    Button B: X+24, Y+48
    Prize: X=6617, Y=5438

    Button A: X+40, Y+16
    Button B: X+12, Y+49
    Prize: X=4232, Y=5052

    Button A: X+62, Y+30
    Button B: X+12, Y+49
    Prize: X=17510, Y=15422

    Button A: X+80, Y+72
    Button B: X+13, Y+85
    Prize: X=5774, Y=7982

    Button A: X+17, Y+36
    Button B: X+76, Y+30
    Prize: X=3583, Y=2088

    Button A: X+42, Y+16
    Button B: X+26, Y+49
    Prize: X=15122, Y=6635

    Button A: X+53, Y+24
    Button B: X+14, Y+49
    Prize: X=11581, Y=18716

    Button A: X+86, Y+46
    Button B: X+11, Y+52
    Prize: X=15118, Y=2046

    Button A: X+27, Y+82
    Button B: X+57, Y+14
    Prize: X=5331, Y=2666

    Button A: X+32, Y+79
    Button B: X+57, Y+13
    Prize: X=11138, Y=17121

    Button A: X+76, Y+27
    Button B: X+16, Y+66
    Prize: X=19352, Y=13364

    Button A: X+74, Y+21
    Button B: X+27, Y+49
    Prize: X=6446, Y=2408

    Button A: X+58, Y+30
    Button B: X+35, Y+65
    Prize: X=12741, Y=2295

    Button A: X+42, Y+18
    Button B: X+31, Y+65
    Prize: X=1149, Y=16831

    Button A: X+56, Y+52
    Button B: X+25, Y+88
    Prize: X=6615, Y=10224

    Button A: X+16, Y+55
    Button B: X+32, Y+21
    Prize: X=2672, Y=3934

    Button A: X+20, Y+38
    Button B: X+87, Y+27
    Prize: X=4930, Y=2452

    Button A: X+26, Y+41
    Button B: X+46, Y+22
    Prize: X=6082, Y=11236

    Button A: X+25, Y+38
    Button B: X+50, Y+13
    Prize: X=2825, Y=892

    Button A: X+33, Y+12
    Button B: X+23, Y+41
    Prize: X=19025, Y=17153

    Button A: X+48, Y+92
    Button B: X+76, Y+36
    Prize: X=5436, Y=8116

    Button A: X+48, Y+26
    Button B: X+30, Y+64
    Prize: X=2868, Y=2222

    Button A: X+22, Y+74
    Button B: X+70, Y+17
    Prize: X=9994, Y=14649

    Button A: X+51, Y+59
    Button B: X+14, Y+83
    Prize: X=1295, Y=5974

    Button A: X+64, Y+13
    Button B: X+23, Y+66
    Prize: X=13619, Y=10648

    Button A: X+46, Y+94
    Button B: X+43, Y+21
    Prize: X=1762, Y=1862

    Button A: X+42, Y+73
    Button B: X+35, Y+14
    Prize: X=1866, Y=11520

    Button A: X+29, Y+52
    Button B: X+30, Y+11
    Prize: X=9581, Y=2018

    Button A: X+69, Y+74
    Button B: X+91, Y+19
    Prize: X=1914, Y=638

    Button A: X+36, Y+18
    Button B: X+41, Y+65
    Prize: X=14203, Y=4129

    Button A: X+58, Y+19
    Button B: X+21, Y+68
    Prize: X=12321, Y=5298

    Button A: X+30, Y+77
    Button B: X+61, Y+14
    Prize: X=16876, Y=13962

    Button A: X+19, Y+57
    Button B: X+57, Y+18
    Prize: X=1906, Y=7763

    Button A: X+41, Y+13
    Button B: X+22, Y+54
    Prize: X=12351, Y=3275

    Button A: X+22, Y+64
    Button B: X+41, Y+12
    Prize: X=14891, Y=16492

    Button A: X+25, Y+64
    Button B: X+62, Y+24
    Prize: X=19382, Y=9816

    Button A: X+11, Y+32
    Button B: X+62, Y+33
    Prize: X=13818, Y=13270

    Button A: X+13, Y+47
    Button B: X+70, Y+40
    Prize: X=4058, Y=5822

    Button A: X+31, Y+57
    Button B: X+52, Y+31
    Prize: X=2796, Y=1975

    Button A: X+28, Y+81
    Button B: X+61, Y+14
    Prize: X=3748, Y=5668

    Button A: X+76, Y+22
    Button B: X+11, Y+94
    Prize: X=3577, Y=4214

    Button A: X+89, Y+11
    Button B: X+94, Y+95
    Prize: X=11414, Y=4746

    Button A: X+20, Y+46
    Button B: X+49, Y+28
    Prize: X=14532, Y=5270

    Button A: X+22, Y+13
    Button B: X+34, Y+80
    Prize: X=3288, Y=7155

    Button A: X+72, Y+27
    Button B: X+14, Y+59
    Prize: X=15070, Y=9175

    Button A: X+21, Y+16
    Button B: X+26, Y+68
    Prize: X=690, Y=1104

    Button A: X+20, Y+52
    Button B: X+61, Y+42
    Prize: X=4553, Y=5658

    Button A: X+78, Y+28
    Button B: X+17, Y+62
    Prize: X=5456, Y=18696

    Button A: X+17, Y+60
    Button B: X+63, Y+26
    Prize: X=1473, Y=11584

    Button A: X+79, Y+56
    Button B: X+16, Y+80
    Prize: X=2106, Y=3072

    Button A: X+54, Y+19
    Button B: X+14, Y+36
    Prize: X=5404, Y=2150

    Button A: X+55, Y+15
    Button B: X+28, Y+55
    Prize: X=7861, Y=16040

    Button A: X+69, Y+48
    Button B: X+41, Y+88
    Prize: X=5242, Y=3944
    """;
}
