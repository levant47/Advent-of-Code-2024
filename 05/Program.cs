public static class Program
{
    ...
    {
        Console.WriteLine("Starting problem 05...");

        var config = UpdateConfiguration.Parse(INPUT);
        ...
        foreach (var update in config.Updates)
        {
            for (var i = 0; i < update.Count; i++)
            {
                foreach (var rule in config.Rules)
                {
                    ...
                    {
                        for (var k = i + 1; k < update.Count; k++)
                        {
                            if (update[k] == rule.First)
                            {
                                wrongUpdates.Add(update);
                                goto NextUpdate;
                            }
        ...
        Console.WriteLine($"Part 1 answer: {safeUpdates.Sum(update => update[update.Count / 2])}");

        var correctedUpdates = new List<List<int>>();
        foreach (var update in wrongUpdates)
        {
            var corrected = update.ToList();
            var moved = false;
            do
            {
                moved = false;
                foreach (var rule in config.Rules)
                {
                    var firstIndex = corrected.IndexOf(rule.First);
                    if (firstIndex == -1) { continue; }
                    var secondIndex = corrected.IndexOf(rule.Second);
                    if (secondIndex == -1) { continue; }
                    if (firstIndex > secondIndex)
                    {
                        corrected.RemoveAt(firstIndex);
                        corrected.Insert(secondIndex, rule.First);
                        moved = true;
                    }
                }
            } while (moved);
        ...
        Console.WriteLine($"Part 2 answer: {correctedUpdates.Sum(update => update[update.Count / 2])}");

        Console.WriteLine("Done");
    }

    public struct PageOrderingRule
    {
        public int First;
        public int Second;
    }
    ...
    {
        public List<PageOrderingRule> Rules;
        public List<List<int>> Updates;

        public static UpdateConfiguration Parse(string source)
        {
            var result = new UpdateConfiguration { Rules = [], Updates = [] };
            ...
            foreach (var line in source.Split('\n').Select(line => line.Trim()))
            {
                if (firstSection)
                {
                    ...
                    var first = int.Parse(words[0]);
                    var second = int.Parse(words[1]);
                    result.Rules.Add(new() { First = first, Second = second });
                }
                else
                {
